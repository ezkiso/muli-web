...
export default function NicolapsoLoginPage() {
  ...
  async function checkIfAlreadyAuthenticated() {
    try {
      ...
      if (user) {
        ...
        // console.log('Login - Owned stores:', ownedStores)
        // if (ownedStores.includes(STORE_ID)) {
        //   console.log('Login - User is owner, redirecting to admin')
        //   router.push(`/stores/${STORE_ID}/admin`)
        // } else {
        //   console.log('Login - User not owner, staying on login')
        // }
        console.log('Login - User authenticated, redirecting to admin')
        router.push(`/stores/${STORE_ID}/admin`)
      }
    } catch (err) {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      const success = await signInNicolapso(email, password)
      if (success) {
        router.push(`/stores/${STORE_ID}/admin`)
      } else {
  ...
  const primaryColor = storeConfig?.primaryColor || '#F59E0B'
